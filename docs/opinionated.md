...
本仓库是**有立场的**工具包。本文档坦诚说明哪些部分是通用的、哪些是作者个人的风味、如何按你的团队改写。
...
以下是基于作者个人一线实践的具体选择，带有作者个人偏好：

### 1. PRD 骨架

- **7 章模块级骨架**（`templates/default-style/module-prd-7-chapter.md`）— 章节编号、顺序、命名都是作者约定
- **10 章功能级骨架** — 包含"追溯矩阵"等作者约定的章节
...
把所有功能切成"操作型 / 页面型 / 执行型"三类，各有固定模板。这是本仓库采用的分类。
...
改 `templates/default-style/` 下的骨架即可：

```bash
# 复制一份作为起点
cp -R templates/default-style templates/your-company-style
...
本仓库的 opinionation 来自作者个人的实践。它可能不适合你。有几种你可以做的选择：
